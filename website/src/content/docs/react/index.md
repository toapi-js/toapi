...
`@toapi/react` connects the [`@toapi/client`](/toapi/client/) fetch client to React. It exposes a single hook, [`useQuery`](/toapi/react/reference/use-query/), that reads a query from the client and keeps the rendered value up to date as the underlying cache entry changes.

The client's `.get()` calls already return a promise that is also an [`Observable`](/toapi/client/reference/observable/): it can be awaited like any promise, and it can be subscribed to for live updates. `useQuery` bridges both halves into React. It hands the promise to React's `use()` so the component suspends until data is available, and it subscribes to the client's internal PubSub so that when the entry is revalidated — through Toapi's [tag-based cache invalidation](/toapi/client/reference/revalidation/) or a manual `client.books.revalidate()` — the component re-renders with the fresh data. No extra query cache, store, or provider is required.
...
- [`@toapi/client`](/toapi/client/) — the source of the queries you pass to `useQuery`.
- `react` — version 19 or newer, because `useQuery` relies on the `use()` hook and Suspense.

## Quick start

Create a client once (see [`createFetchClient`](/toapi/client/reference/create-fetch-client/)), then read from it inside a component with `useQuery`.
...
The value you pass to `useQuery` is whatever `client.<path>.get()` returns — a promise that also implements [`Observable`](/toapi/client/reference/observable/). `useQuery` uses both facets:
...
See [useQuery](/toapi/react/reference/use-query/) for the full signature and options.

## Public exports

| Export | Kind | Reference |
| --- | --- | --- |
| `useQuery` | hook | [useQuery](/toapi/react/reference/use-query/) |
