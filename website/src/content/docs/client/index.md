...
See [createFetchClient](/toapi/client/reference/create-fetch-client/) for the full reference.
...
- **`.get(query?, req?)`** — performs a `GET`. The first argument is the query object (required only if the route declares required query params); the second is a standard `RequestInit`. Returns a promise that is also an [`Observable`](/toapi/client/reference/observable/).
- **`.post(body?, req?)` / `.put(body?, req?)` / `.patch(body?, req?)`** — performs a mutation with a JSON or `FormData` body. Query params go on `req.query`. Returns a promise augmented with `.revalidated` (see [Revalidation & subscriptions](/toapi/client/reference/revalidation/)).
...
See [Revalidation & subscriptions](/toapi/client/reference/revalidation/) for details.

## Subscriptions

The promise returned by `.get()` also implements [`Observable`](/toapi/client/reference/observable/): call `.subscribe(callback)` to be notified whenever the cache entry for that URL changes. This is the primitive that [`@toapi/react`](/toapi/react/) builds its hooks on, but you can use it directly with any state-management approach.
...
Non-`2xx` responses reject with an [`HttpError`](/toapi/client/reference/http-error/) carrying the status code and any structured error payload. Validation failures reported by the server (`application/json+zodissues`) reject with a `ZodError`.

## Public exports

| Export | Kind | Reference |
| --- | --- | --- |
| `createFetchClient` | function | [createFetchClient](/toapi/client/reference/create-fetch-client/) |
| `Client` | type | [createFetchClient](/toapi/client/reference/create-fetch-client/) |
| `Observable` | type | [Observable](/toapi/client/reference/observable/) |
| `GetRoute`, `PostRoute` | types | [Route types](/toapi/client/reference/route-types/) |
| `HttpError` | class (re-export) | [HttpError](/toapi/client/reference/http-error/) |
| `INVALIDATIONS_ROUTE` | constant (re-export) | [Revalidation & subscriptions](/toapi/client/reference/revalidation/) |
| `Logger` | type (re-export) | [createFetchClient](/toapi/client/reference/create-fetch-client/) |
