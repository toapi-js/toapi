...
| `minTTL`           | `number`                                                | `100`                               | Milliseconds within which repeated invalidations of the same URL are debounced into a single revalidation. Also the delay before a failed entry is evicted from the cache. |
