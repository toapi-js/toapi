...
`createRequestHandler` is the primary server entry point. It takes the [`ApiDefinition`](/toapi/server/reference/api-definition/) produced by [`defineApi`](/toapi/server/reference/define-api/) and returns an async function that maps a standard Web API `Request` to a `Response`. Because it works with the Web-standard `Request`/`Response` objects, the returned handler drops straight into Astro, Next.js, Hono, Bun, Deno, Cloudflare Workers, or any other Web-standard runtime — no adapter required.
...
See the framework guides for how to wire the handler into a specific runtime: [Astro](/toapi/server/guides/astro/), [Next.js](/toapi/server/guides/nextjs/), [Hono](/toapi/server/guides/hono/), [Bun](/toapi/server/guides/bun/).
...
The API definition object returned by [`defineApi`](/toapi/server/reference/define-api/), containing the complete map of routes, the cache instance, the optional OpenAPI info, and the optional logger.
...
Route paths registered with [`defineApi().route()`](/toapi/server/reference/define-api/) support three segment kinds, compiled to a regular expression at startup:
...
| --------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `<basePath>/__tapi/invalidations` | always                                | Opens the long-polling invalidation stream via [`streamRevalidatedTags`](/toapi/server/reference/stream-revalidated-tags/). Clients and service workers connect here to receive revalidated tags. |
| `<basePath>/__tapi/openapi.json`  | only when `oas` is set on `defineApi` | Serves the generated OpenAPI document (see [`generateOpenAPISchema`](/toapi/server/reference/generate-openapi-schema/)). The document is generated once and cached in memory.                     |

:::note
Because the invalidation stream is mounted automatically, you do not need a separate route for it when you use `createRequestHandler`. You only wire up [`streamRevalidatedTags`](/toapi/server/reference/stream-revalidated-tags/) by hand when your framework handles that path outside the Toapi handler.
:::
...
1. **Prepare the request.** The incoming `Request` is augmented into a [`TRequest`](/toapi/server/reference/t-request/), giving lazy, validated accessors for `params()`, `query()`, `cookies()`, `data()`, and `invalidate()`.
...
4. **Run the handler.** The handler function executes and returns a [`TResponse`](/toapi/server/reference/t-response/). If a `response` schema is defined, the returned data is validated against it.
...
Errors thrown while handling a request are logged (via the [`Logger`](/toapi/server/reference/define-api/#logger-interface) supplied to `defineApi`, or `console.error`) and converted into responses:
...
| [`HttpError`](/toapi/server/reference/http-error/) | The error's `status`, body `{ message, data }`, `Content-Type: application/json+httperror`.     |
...
- [`defineApi`](/toapi/server/reference/define-api/) — build the API definition passed in here.
- [Caching Strategies](/toapi/server/reference/caching/) — how the cache layers cooperate.
