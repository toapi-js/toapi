...
`streamRevalidatedTags` returns a streaming `Response` that pushes revalidated cache tags to a connected client over a long-lived connection. Service workers and clients subscribe to this stream to learn when tagged resources become stale, so they can refresh cached data immediately (see [Caching Strategies](/toapi/server/reference/caching/)).

:::note
When you serve your API with [`createRequestHandler`](/toapi/server/reference/create-request-handler/), this stream is already mounted at `<basePath>/__tapi/invalidations`. You only call `streamRevalidatedTags` directly when your framework routes that path outside the Toapi handler.
:::
...
**Type**: [`Cache`](/toapi/server/reference/pub-sub/)
...
- [`createRequestHandler`](/toapi/server/reference/create-request-handler/) — mounts this stream automatically.
- [`PubSub` and the `Cache` interface](/toapi/server/reference/pub-sub/) — the pub/sub contract behind the stream.
- [Caching Strategies](/toapi/server/reference/caching/) — how invalidations propagate to each cache layer.
